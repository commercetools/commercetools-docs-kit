...
- `gatsby-config.js`: this is required for a Gatsby website and should contain the website specific configuration. At the very least, the commercetools Gatsby theme must be listed in the plugins section. You are free to provide more plugins as you need for your specific website.
  ...
  - `overrideTopMenuLinks` (_optional_): allows to override the top menu configuration files by providing a `top-menu.yaml` and `top-side-menu.yaml` files in the website `src/data` folder. This would be useful when testing out new links without having to change the default ones. **Default: `false`**
