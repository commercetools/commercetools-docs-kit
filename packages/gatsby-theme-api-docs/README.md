...
For types, the links are generated using our custom URN - `ctp:<api-key>:type:<type-name>`.
...
For endpoints, the links are generated using our custom URN - `ctp:<api-key>:endpoint:<end-point>:<method>`. Note that `<end-point>` should includes URI encoded characters for `{`, `}`, `#`, `:`, etc.
