...
- `products` (**required**, array of strings): Defines which products the website relates to. Available options are: [`Composable Commerce`, `Frontend`, `Checkout`, `Connect`].
- `contentType` (**required**, string): Defines the type of content in the website. Available options are: `referenceDocs`, `guidedDocs`, `userDocs`.
