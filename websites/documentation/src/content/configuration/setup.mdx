...
It is the core [Gatsby theme](https://www.gatsbyjs.com/docs/themes/) for building commercetools documentation websites.
...
- `gatsby-config.js`: this is required for a Gatsby website and should contain the website specific configuration. If your website requires the usage of [add-ons](/configuration/extensions#using-theme-with-add-ons), you need to configure the main theme using the `configureThemeWithAddOns` helper function. See examples below.
