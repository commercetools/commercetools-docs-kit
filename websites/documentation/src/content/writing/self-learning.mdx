...
- `submitAttempt`: It's the main function, it takes three parameters:
  ...
  - `finish`: boolean which dictate if the attempt should be considered finished after subission
- `isLoading`: It's a boolean value indicating if the submit operation is still in progress (if `true`)
- `error`: In case of error, this property will contain error information. The format of error might depend on its type.
- `correlationId`: If the API returns a response including a `X-Correlation-ID` value, this property gets its value.
...
The component doesn't recieves any props and it's just injected at the root of the application.
...
The profile modal visibility is bound to a value in the LearningContext and the correct way to open it or close it is by useCourseInfoByPageSlugs
...
`<ProfileModal />` component is injected in the root of the application, its default state is **closed**.
...
The component expets the following props:
