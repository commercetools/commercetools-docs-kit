...
- `<SideBySide>`<Anchor name="side-by-side" />: use to wrap two content blocks side by side on wide viewport sizes. This feature is enabled by setting the `wideLayout` [frontmatter option](#wide-layout).
