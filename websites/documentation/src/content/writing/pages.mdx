...
- `showTimeToRead` (boolean)<Anchor name="show-time-to-read" />: use to show the reading time of a page. If the `setTimeToRead` frontmatter is not defined, gatsby will analyse the page and determine the reading time.

- `setTimeToRead` (number)<Anchor name="set-time-to-read" />: use to set the reading time of a page. If defined, it replaces the automatically generated value form gatsby.
