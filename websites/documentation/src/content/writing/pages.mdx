...
- `<SideBySide>`<Anchor name="side-by-side" />: use to wrap two content blocks side by side on wide viewport sizes. This feature is enabled by configuring the `allowWideContentLayout` site metadata option together with `wideLayout` [frontmatter option](#wide-layout).
