...
- `navLevels` (number): use to reduce the depth of right-hand index navigation for pages where it could get too long to fit all headings in the screen. You must set to 2. For more information, see [Child section navigation](/writing/navigation#child-section-navigation).
...
- `<Subtitle>`: use to provide a brief information about the page besides the title. For more information, see [Titles and headings](/writing/markdown#titles-and-headings).
...
- `<SideBySide>`<Anchor name="side-by-side" />: use to wrap two content blocks side by side on wide viewport sizes. This feature is enabled by configuring the `allowWideContentLayout` theme option or `wideLayout` [frontmatter option](#wide-layout).
