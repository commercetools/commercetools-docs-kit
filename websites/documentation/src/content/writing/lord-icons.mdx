...
In ui-kit package we exposes a wrapper component that enables developers to include lordicon icons with the minumum effort and supporting all the native functionalities of lordicons. The component supports SSR by displaying a static frame of the animation (it needs to be provided in svg format) until the client side animation loads.
