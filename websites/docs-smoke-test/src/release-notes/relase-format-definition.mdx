 ...
 * `type`: A text; must be either "New Feature" or "Resolved Issue" or "Enhancement"
 * `topics`: YAML array of text entries
 * `description`: Max 256 characters plain text containing the change and its advantage ("tweet-able" and for RSS).
