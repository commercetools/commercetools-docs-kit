---
date: 2000-02-02
title: Test Excerpt Rendering
description: |
  This release exposes the most common Markdown syntax before and after the read more separator to test rendering
type: fix
topics:
  - Testing
  - Rendering
---

<Subtitle>Subtitle Text</Subtitle>

A plain paragraph `with inline code` in it. And **bold** and *italic*.

# Heading 1

 * Bullet List
 * Bullet List 2

 1. Ordered List 1
 2. Ordered List 2

## Heading 2

> A blockquote piece

```sh
Code in a fenced code block
```

### Heading 3

| table head 1 | table head 2 |
|---|---|
| table call 1A | table cell 2A |
| table cell 2A | table cell 2B |

<dl>
  <dt>Term</dt>
  <dd>Definition of the Term</dd>
</dl>

An Image with a caption:

![the image alt text goes here](/images/releases/1200x300.jpg 'The image title goes here and should be rendered as the caption')

An Image without a caption:

![the image alt text goes here](/images/releases/subfolder-in-images/1200x300.jpg)

An Image with a transparent background:

![the image alt text goes here](/images/releases/transparent-bkg.png)

A small image:

![the image alt text goes here](/images/releases/200.jpg)

separator line below this

----

<!--more-->

----

separator line above this

<Subtitle>Subtitle Text</Subtitle>

A plain paragraph `with inline code` in it. And **bold** and *italic*.

# Heading 1

 * Bullet List
 * Bullet List 2

 1. Ordered List 1
 2. Ordered List 2

## Heading 2

> A blockquote piece

```sh
Code in a fenced code block
```
### Heading 3

| table head 1 | table head 2 |
|---|---|
| table call 1A | table cell 2A |
| table cell 2A | table cell 2B |

<dl>
  <dt>Term</dt>
  <dd>Definition of the Term</dd>
</dl>
